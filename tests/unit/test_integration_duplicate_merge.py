...
def test_integration_duplicate_merge():
    ...
    subtitles = [
        SubtitleItem(index=1, start_ms=0, end_ms=1200, text="さて図書館行って、病院行って、 銀行にも行ってくるでは出発です"),
        SubtitleItem(index=2, start_ms=2000, end_ms=11200, text="さて図書館行って、病院行って、銀行にも行ってくるでは出発です"),
        SubtitleItem(index=3, start_ms=16000, end_ms=27200, text="汗だくで帰宅しました、シャワー浴びてきたのでスッキリ凍らせた水持って出かけたけど真夏の外出は危険だと思った"),
        SubtitleItem(index=4, start_ms=22000, end_ms=25200, text="汗だくで帰宅しました、シヤワー浴びてきたのでスッキリ凍らせた水持って出かけたけど真夏の外出は危険だと思った"),
        SubtitleItem(index=5, start_ms=30000, end_ms=39200, text="がん検診の検査結果は異常なしでしたこれからも年1ペ一スで受けたいです"),
        SubtitleItem(index=6, start_ms=42000, end_ms=53200, text="お昼ごはんはカレー蕎麦とネギトロ巻きにします午後にはまた大量のネギトロが届くらしいので消化していかないと"),
        SubtitleItem(index=7, start_ms=44000, end_ms=45200, text="お昼ごはんはカレー蕎麦とネギトロ巻きにします午後にはまた大量のネギト口が届くらしいので消化していかないと"),
        SubtitleItem(index=8, start_ms=60000, end_ms=65200, text="なんかスパッと切れなくてボロボロになっていく"),
        SubtitleItem(index=9, start_ms=70000, end_ms=71200, text="昨日作った力レーが微妙に残ってたので出汣で伸ばしていきます"),
    ]
...
def test_time_constraint_behavior():
    ...
    try:
        ...
        if REAL_CODE_AVAILABLE:
            # 時間制約により統合されないことを確認
            assert len(merged_subtitles) == 3, f"時間制約により統合されないはず: {len(merged_subtitles)}"

...
class TestSettingsManager:
    ...
    def test_save_and_load_settings(self, mock_settings_manager):
        ...
        custom_settings = AppSettings(
            extraction=ExtractionSettings(fps_sample=5.0, ocr_confidence=0.9),
            formatting=FormattingSettings(max_chars=50, max_lines=3),
            output=OutputSettings(encoding="UTF-8 BOM", srt_bom=True),
            ui=UISettings(theme="ダーク", font_size=12),
        )
    ...
    def test_settings_validation(self, mock_settings_manager):
        ...
        invalid_settings = AppSettings(
            extraction=ExtractionSettings(
                fps_sample=15.0, ocr_confidence=1.5  # 上限を超える  # 範囲外
            ),
            formatting=FormattingSettings(
                max_chars=5, max_lines=15  # 下限を下回る  # 上限を超える
            ),
            output=OutputSettings(output_folder="/nonexistent/path"),
            ui=UISettings(),
        )

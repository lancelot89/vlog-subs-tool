...
class TestCSVExportImportIntegration:
    ...
    def test_translated_csv_import(self, sample_translated_csv_content, sample_subtitles_japanese):
        ...
        try:
            ...
            # 結果の検証
            if result.success:
                assert len(result.subtitles) == 5, f"期待される字幕数と異なる: {len(result.subtitles)}"
                ...
                if len(result.subtitles) > 0:
                    ...
                    assert "Hello, World!" in first_subtitle.text, f"翻訳テキストが正しくない: '{first_subtitle.text}'"
    ...
    def test_csv_encoding_handling(self, sample_subtitles_japanese):
        ...
        for encoding in encodings_to_test:
            ...
            try:
                ...
                assert csv_path.exists(), f"エンコーディング '{encoding}' でファイルが作成されていない"

                # 文字化けしていないことを確認
                with open(csv_path, 'r', encoding=encoding) as f:
                    content = f.read()
                    assert "こんにちは" in content, f"エンコーディング '{encoding}' で文字化けが発生"
    ...
    def test_csv_validation_and_error_handling(self):
        ...
        with pytest.raises(FileNotFoundError):
            importer.import_translated_csv(Path("nonexistent_file.csv"), [])
    ...
    def test_gas_compatible_format(self, sample_subtitles_japanese):
        ...
        try:
            ...
            # ヘッダー行の確認
            assert len(lines) >= 1, "CSVファイルにヘッダーがない"
            header = lines[0].strip()

            # 基本的な列が含まれていることを確認
            assert "時間" in header or "Time" in header, "時間列が含まれていない"
    ...
    def test_batch_translation_workflow(self, sample_subtitles_japanese):
        ...
        try:
            ...
            # Step 4: 結果の検証
            if result.success:
                assert len(result.subtitles) == len(sample_subtitles_japanese), "翻訳後の字幕数が一致しない"

                # 翻訳されたテキストの確認
                if len(result.subtitles) > 0:
                    assert "Hello, World!" in result.subtitles[0].text, "1番目の翻訳が正しくない"
                if len(result.subtitles) > 1:
                    assert "This is a test." in result.subtitles[1].text, "2番目の翻訳（複数行）が正しくない"

...
class SubtitleCSVImporter:
    ...
    def import_translated_csv(
        self, filepath: Path, original_subtitles: List[SubtitleItem]
        ...
        try:
            ...
            for row_index, row in enumerate(data_rows, start=header_row_index + 2):  # 行番号は1ベース + ヘッダー行
                try:
                    # メタデータ行やコメント行をスキップ
    ...
    def _detect_language_from_filename(self, filepath: Path) -> str:
        ...
        lang_patterns = {
            ...
            "zh": [r"(?:^|[_\-\s])zh(?:[_\-\s]|$)", r"(?:^|[_\-\s])chinese(?:[_\-\s]|$)", r"中文", r"中国語"],
            "ko": [r"(?:^|[_\-\s])ko(?:[_\-\s]|$)", r"(?:^|[_\-\s])korean(?:[_\-\s]|$)", r"한국어", r"韓国語"],
            "es": [r"(?:^|[_\-\s])es(?:[_\-\s]|$)", r"(?:^|[_\-\s])spanish(?:[_\-\s]|$)", r"español", r"スペイン語"],
            "fr": [r"(?:^|[_\-\s])fr(?:[_\-\s]|$)", r"(?:^|[_\-\s])french(?:[_\-\s]|$)", r"français", r"フランス語"],
            "de": [r"(?:^|[_\-\s])de(?:[_\-\s]|$)", r"(?:^|[_\-\s])german(?:[_\-\s]|$)", r"deutsch", r"ドイツ語"],
            "ar": [r"(?:^|[_\-\s])ar(?:[_\-\s]|$)", r"(?:^|[_\-\s])arabic(?:[_\-\s]|$)", r"عربي", r"アラビア語"],
        }
    ...
    def _create_subtitle_from_row(
        ...
        try:
            # 字幕番号から元データを特定
            subtitle_index = (
                int(row[indices.get("index", 0)])
                if indices.get("index") is not None
                else row_index - 1
            )
    ...
    def _create_standard_subtitle_from_row(
        self, row: List[str], headers: List[str], row_index: int
    ) -> Optional[SubtitleItem]:
        """標準CSVから字幕アイテムを作成"""
        try:
            indices = self._get_column_indices(headers)
    ...
    def _validate_translation(
        self, subtitle: SubtitleItem, row_index: int, result: TranslationImportResult
    ) -> bool:
        """翻訳の妥当性チェック"""
...
class CSVWorkflowValidator:
    ...
    @staticmethod
    def validate_translation_workflow(
            ...
            for trans_file in translated_files:
                ...
                if trans_file.exists():
                    ...
                    result["warnings"].append(
                        f"{lang} 翻訳ファイルが見つかりません: {trans_file.name}"
                    )

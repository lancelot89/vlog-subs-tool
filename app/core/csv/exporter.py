...
class SubtitleCSVExporter:
    ...
    def export_for_translation(
        self, subtitles: List[SubtitleItem], filepath: Path, source_lang: str = "ja"
    ) -> bool:
        """
...
class TranslationWorkflowManager:
    ...
    def create_translation_workflow(
        self, subtitles: List[SubtitleItem], video_name: str, target_langs: List[str]
    ) -> Dict[str, Path]:
        """
        翻訳ワークフロー用ファイル群を作成
    ...
    def _create_translation_template(
        self, subtitles: List[SubtitleItem], filepath: Path, target_lang: str
    ) -> bool:
        """翻訳テンプレートファイル作成"""
        try:
            exporter = SubtitleCSVExporter()

...
class LineLengthRule(QCRule):
    ...
    def __init__(self, max_chars: int = 42, enabled: bool = True):
        super().__init__(
            name="行長チェック",
            description=f"1行{max_chars}文字以内",
            enabled=enabled
        )
...
class DurationRule(QCRule):
    ...
    def __init__(self, min_duration_ms: int = 1200, max_duration_ms: int = 10000, enabled: bool = True):
        super().__init__(
            name="表示時間チェック",
    ...
    def check(self, subtitles: List[SubtitleItem]) -> List[QCResult]:
        ...
        for i, subtitle1 in enumerate(subtitles):
            for j, subtitle2 in enumerate(subtitles[i+1:], i+1):
                # 時間重複の判定
                if (subtitle1.start_ms < subtitle2.end_ms and 
                    subtitle1.end_ms > subtitle2.start_ms):
...
class QCChecker:
    ...
    def filter_results(self, results: List[QCResult], severity: QCSeverity = None) -> List[QCResult]:
        """重要度でフィルタリング"""
        if severity is None:
            return results

...
class ROIDetector:
    ...
    def visualize_roi(self, frame: np.ndarray, roi: ROIRegion, color=(0, 255, 0), thickness=2) -> np.ndarray:
        """ROI領域を可視化"""
...
class AutoROIDetector(ROIDetector):
    ...
    def _is_subtitle_like_region(self, x: int, y: int, w: int, h: int, frame_shape: Tuple[int, int, int]) -> bool:
        """字幕らしい領域かどうかを判定"""
    ...
    def _find_consistent_roi(self, text_regions: List[ROIRegion]) -> ROIRegion:
        ...
        if not text_regions:
            # フォールバック
            return ROIRegion(0, int(self.frame_height * 0.7), self.frame_width, int(self.frame_height * 0.3))
...
class ManualROIDetector(ROIDetector):
    ...
    def __init__(self, frame_width: int, frame_height: int, manual_rect: Tuple[int, int, int, int]):
        super().__init__(frame_width, frame_height)
